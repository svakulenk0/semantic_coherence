...
def get_maximum_similarities_distribution(sample=SAMPLE_WORDS_4606, entities_cosines=cosines):
    '''
    ...
    return Counter([np.max(enity_cosine) for enity_cosine in entities_cosines])
...
def collect_word_distances(type, sample='291848'):
    embeddings = load_GloVe_embeddings()
    # load training data
    # positive samples
    positives = np.load('./%s/words/%s_X.npy' % (sample, ))
...
def compare_distance_distributions():
    ...
    print get_maximum_similarities(positive_distances)
    print get_maximum_similarities(random_distances)
