...
def collect_distances(embeddings, samples_type, path):
    # load data
    samples = np.load(path+'data/%s_X.npy'%samples_type)
...
def compare_entity_distance_distributions(sample='291848'):
    ...
    compare word distance distributions in dialogues
    '''
    path = './%s/entities/' % sample
    embeddings = np.load(path+'embeddings/rdf2vec_PageRankSplit.npy')
    compare_distance_distributions(embeddings, path)
